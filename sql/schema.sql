...
CREATE TABLE IF NOT EXISTS admins (
  ...
  role          VARCHAR(20) NOT NULL DEFAULT 'admin',  -- 'admin' = full access, 'moderator' = view + comment moderation only
  created_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
-- If this table already existed before "role" was added, this adds the
-- column safely without touching existing rows (existing admins stay full-access).
